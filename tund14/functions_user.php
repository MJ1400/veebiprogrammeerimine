<?php
  ...
  $stmt = $conn->prepare("SELECT id FROM vpuserprofiles WHERE userid=?");
    ...
    if($stmt->execute()) {
      ...
      $notice = "Profiili salvestamisel tekkis tõrge!";
    }
  ...
  function changePassword($passwordNew, $password) {
    $notice = "";
    $conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    ...
    if($stmt->execute()) {
      ...
      if($stmt->fetch()){
        ...
        if(password_verify($password, $passwordFromDb)) {
         ...
         if($stmt->execute()) {
           $notice = "Parooli muutmine õnnestus!";
         } else {
           $notice = "Parooli muutmisel tekkis viga!:" .$stmt->error;
         }


       } else {
         $notice = "Pragune parool ei klapi andmebaasi parooliga!";
       }
